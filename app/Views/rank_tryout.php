            ...
            <table class="table table-hover text-nowrap">
                ...
                <tbody>
                    <?php foreach ($participants as $i => $participant) : ?>
                        <tr>
                            <td><?= ($i + 1) ?></td>
                            <td><a href="<?= base_url("tryout/$exam->id/participant/$participant->id") ?>" class="text-reset"><b><?= $participant->name ?></a></b></td>
                            <td><?= $participant->score_tps ?></td>
                            <td><?= $participant->score_tka ?></td>
                            <td><?= $participant->score_agregate ?></td>
                            <td>
                                <a href="<?= base_url("tryout/$exam->id/participant/$participant->id") ?>" class="badge badge-info">DETAIL</a>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                </tbody>
